...
void cadastrarAtendimento(struct cliente clientes[], int *totalClientes);
void listarAtendimentos(struct cliente clientes[], int totalClientes);
void listarPorTipo(struct cliente clientes[], int totalClientes, int tipo);

int main() {
	setLocale(LC_ALL, "Portuguese");
	//Inicialização de variáveis
	struct cliente clientes[100]; //supondo limite de 100 clientes
	int totalClientes = 0;
	int escolha;
	int tipo;
	do{
